...
- `cargo test --locked --workspace`: 42 passed, no failures; one separately
...
- Final current-binary isolated network acceptance: two user/net namespaces,
  ...
  CLI resume, and matching SHA-256. The shorter exact-tree run is recorded in
  `target/validation/network-interruption-final.log`; the exact-tree 2 GiB run is recorded in
  `target/validation/network-interruption-final-large.log` with exit code `0`.
- Final current-binary TUI smoke and generated Linux user-service test passed;
...
All command output and exit codes are retained as matching `.log` and `.exit`
files under `target/validation/`. The release package files are under
`target/packages/` and are local build artifacts, not published releases.
