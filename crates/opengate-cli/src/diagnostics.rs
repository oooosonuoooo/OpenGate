...
pub async fn run(dir: &Path, device: Option<String>) -> Result<Value> {
    let authentication = if let Some(device) = device {
        ...
        match result {
            Ok(reply) => json!({"authenticated":true,"control_roundtrip_ms":start.elapsed().as_millis(),"remote":reply.data}),
            Err(error) => json!({"authenticated":false,"control_roundtrip_ms":start.elapsed().as_millis(),"error":format!("{error:#}")}),
        }
    } else { Value::Null };
    ...
    let paths: Vec<&str> = status["network"]["connections"].as_array().into_iter()
        .flatten().filter_map(|c| c["path"].as_str()).collect();
    ...
    Ok(json!({
        ...
        "packet_loss":{"percent":Value::Null,"reason":"Transport packet-loss counters are not exposed by the current libp2p API; ping failures are reported separately and are not packet-loss measurements."},
