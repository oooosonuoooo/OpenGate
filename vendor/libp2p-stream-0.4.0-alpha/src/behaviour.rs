    ...
    fn on_swarm_event(&mut self, event: FromSwarm) {
        match event {
            FromSwarm::ConnectionEstablished(ConnectionEstablished {
                ...
                // OpenGate: new streams prefer direct paths, then direct QUIC. Existing
                // streams remain on their established connection and are never replayed.
                let priority = if endpoint.is_relayed() { 2 } else if endpoint.get_remote_address()
                    .iter().any(|part| matches!(part, libp2p_core::multiaddr::Protocol::QuicV1)) { 0 } else { 1 };
                Shared::lock(&self.shared).on_connection_established(connection_id, peer_id, priority)
            },
